...
def getAllJobs(request):
    ...
    querset = paginator.paginate_queryset(filterset.qs, request)
    serializer = JobSerializer(querset, many=True)
...
def newJob(request):
    ...
    data = request.data
    job = Job.objects.create(**data)  # **like spread operator in JS
    serializer = JobSerializer(job, many=False)
...
def getTopicStats(request, topic):
    ...
    if len(jobs) == 0:
        return Response({'message': 'No status found for {topic}'.format(topic=topic)})
...
def applyToJob(request, pk):
    ...
    if user.userprofile.resume == '':
        return Response({'error': 'Please upload ypur resume first'}, status=status.HTTP_400_BAD_REQUEST)
...
def getCurrentUserAppliedJobs(request):
    ...
    serializers = CandidatesAppliedSerializer(jobs, many=True)
